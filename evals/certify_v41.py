...
SRC = ROOT / "src"
RESULTS = ROOT / "evals" / "results" / "v4.1"
LINE_GATE = 95.0
BRANCH_GATE = 90.0
STABILITY_RUNS = 5
DIFF_CASES = 100_000
...
def race_stage() -> dict:
    paths = (
        ...
        "tests/test_v41_model_boundary.py", "tests/test_v41_defensive.py", "tests/test_v41_dataset.py",
    )
...
def ai_offline_stage() -> dict:
    p = run(sys.executable, "evals/run_v41_offline.py", check=False, capture=False)
    if p.returncode != 0:
        return {"status": "FAIL", "returncode": p.returncode}
    report_path = ROOT / "evals" / "results" / "v41_offline.json"
    report = json.loads(report_path.read_text())
    return {
        "status": "PASS",
        "dataset_cases": report["dataset"]["cases"],
        "dataset_sha256": report["dataset"]["sha256"],
        "reference_rule_baseline": report["reference_rule_baseline"],
        "planner_boundary_adversarial_fixture": report["planner_boundary_adversarial_fixture"],
        "real_llm_accuracy": report["real_llm_accuracy"],
    }
...
def run_stage(name: str, run_id: int | None) -> int:
    ...
    elif name == "ai-offline": data, label = ai_offline_stage(), "ai-offline"
...
def aggregate() -> int:
    prov = provenance()
    names = ["coverage", *(f"stability-{i}" for i in range(1, 6)), "differential", "races", "performance", "ai-offline", "security", "testmode"]
    ...
    diff=stages["differential"]["data"]; races=stages["races"]["data"]; perf=stages["performance"]["data"]; ai=stages["ai-offline"]["data"]; sec=stages["security"]["data"]; tm=stages["testmode"]["data"]
    offline = all((
        ...
        ai["status"] == "PASS", ai["dataset_cases"] >= 50, ai["planner_boundary_adversarial_fixture"]["unsafe_selection_rate"] == 0.0,
        not sec["forbidden_constructs"], not sec["secret_findings"], git_clean(),
    ))
    metrics={
        "version":"v4.1", "provenance":prov, "coverage":cov,
        ...
        "differential":diff,"races":races,"performance":perf,"ai_offline":ai,"security":sec,"testmode":tm,
        "v4_1_offline_promotion":offline,"real_llm_certified":False,"full_product_certified":False,
    }
    RESULTS.mkdir(parents=True,exist_ok=True); (RESULTS/"metrics.json").write_text(json.dumps(metrics,indent=2,sort_keys=True))
    report=["# AgentCommit V4.1 Structured-AI Offline Promotion","",f"- Git SHA: `{prov['git_sha']}`",f"- Tracked source hash: `{prov['tracked_hash']}`",f"- Tests: **{cov['tests_passed']}**",f"- Coverage: **{cov['line_pct']:.2f}% line / {cov['branch_pct']:.2f}% branch**",f"- Stability: **{sum(x['passed'] for x in stability)}/5**",f"- Differential: **{diff['cases']:,} cases / {diff['mismatches']} mismatches**",f"- Focused races/hardening: **{races['tests_passed']} tests**",f"- p95 V4 commit/dispatch: **{perf['v4_commit_p95_ms']:.2f} / {perf['dispatch_p95_ms']:.2f} ms**",f"- AI held-out dataset: **{ai['dataset_cases']} cases / {ai['dataset_sha256']}**",f"- Planner unsafe-selection rate (adversarial fixture): **{ai['planner_boundary_adversarial_fixture']['unsafe_selection_rate']:.3f}**",f"- Real LLM accuracy: **NOT RUN**",f"- Security findings: **{len(sec['forbidden_constructs'])+len(sec['secret_findings'])}**",f"- Razorpay Test Mode: **{tm['status']}**","","## Decision","",f"**{'PROMOTE V4.1 OFFLINE AI-BOUNDARY RC' if offline else 'DO NOT PROMOTE'}**","","Full product certification remains blocked until a real LLM adapter passes the frozen held-out AI evals and real Razorpay Test Mode Checkout/webhook evidence is completed."]
    (RESULTS/"PROMOTION.md").write_text("\n".join(report)+"\n")
    print(json.dumps(metrics,indent=2)); return 0 if offline else 1


def main(argv: list[str] | None = None) -> int:
    p=argparse.ArgumentParser(); p.add_argument("--stage",choices=["coverage","stability","differential","races","performance","ai-offline","security","testmode"]); p.add_argument("--run-id",type=int); p.add_argument("--aggregate",action="store_true"); a=p.parse_args(argv)
