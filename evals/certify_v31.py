...
SRC = ROOT / "src"
RESULTS = ROOT / "evals" / "results" / "v3.1"
LINE_GATE = 95.0
BRANCH_GATE = 90.0
STABILITY_RUNS = 5
DIFF_CASES = 100_000
...
def performance_stage() -> dict:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for i in range(60):
            m=f"m{i}"; sku=f"s{i}"; q=f"q{i}"; r=f"r{i}"; d=f"d{i}"; g=f"g{i}"; e=f"e{i}"
            ...
            store.reserve(reservation_id=r, quote_id=q, now_ms=now, ttl_ms=60_000)
    ...
    return {
        "samples": len(commit_ms), "commit_p50_ms": statistics.median(commit_ms), "commit_p95_ms": p95(commit_ms),
...
def testmode_stage() -> dict:
    ...
    receipt = deterministic_receipt("v31-cert-" + git_sha()[:12])
...
def aggregate() -> int:
    ...
    offline = all((
        ...
        perf["commit_p95_ms"] < LATENCY_P95_MS, perf["dispatch_p95_ms"] < LATENCY_P95_MS,
        not sec["forbidden_constructs"], not sec["secret_findings"], git_clean(),
    ))
    metrics={
        "version":"v3.1", "provenance":prov, "coverage":cov,
        ...
        "offline_promotion":offline,"full_v31_certified":False,
    }
    RESULTS.mkdir(parents=True,exist_ok=True); (RESULTS/"metrics.json").write_text(json.dumps(metrics,indent=2,sort_keys=True))
    report=["# AgentCommit V3.1 Promotion","",f"- Git SHA: `{prov['git_sha']}`",f"- Tracked source hash: `{prov['tracked_hash']}`",f"- Tests: **{cov['tests_passed']}**",f"- Coverage: **{cov['line_pct']:.2f}% line / {cov['branch_pct']:.2f}% branch**",f"- Stability: **{sum(x['passed'] for x in stability)}/5**",f"- Differential: **{diff['cases']:,} cases / {diff['mismatches']} mismatches**",f"- Focused races/hardening: **{races['tests_passed']} tests**",f"- p95 commit/dispatch: **{perf['commit_p95_ms']:.2f} / {perf['dispatch_p95_ms']:.2f} ms**",f"- Security findings: **{len(sec['forbidden_constructs'])+len(sec['secret_findings'])}**",f"- Test Mode: **{tm['status']}**","","## Decision","",f"**{'PROMOTE OFFLINE RC' if offline else 'DO NOT PROMOTE'}**","","Full V3.1 certification remains blocked until real Standard Checkout + signed webhook + API reconciliation evidence is completed."]
